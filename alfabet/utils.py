...
def CountAtomTypes(mol: Union[Mol, str]):
    """Return a dictionary of each atom type in the given fragment or molecule"""
    if isinstance(mol, str):
        mol = AddHs(MolFromSmiles(smiles, sanitize=True))
        ...
        for bond in mol.GetBonds():
            ...
            if not error:
                try:
                    ...
                    # Validation
                    assert CountAtomTypes(Smi2A) + CountAtomTypes(Smi2B) == CountAtomTypes(smiles), \
                        "Error with {}; {}; {}".format( Smi2A, Smi2B, smiles)
                    ...
                    # Validation
                    assert CountAtomTypes(Smi2A) == CountAtomTypes(smiles), \
                        "Error with {}; {}".format( Smi2A, smiles)
